...
class ControlNode(Node):
    ...
    def timer_callback_2(self):
        if self.start_ep:
            self.act_dict["action"] = self.obs_act_data[self.cfg.total_freedom:]
            self.obs_dict["observation.state"] = self.obs_act_data[:self.cfg.total_freedom]
        ...
        if self.save_ep:
            ...
            invalid_diffs_1 = [i for i, ts in enumerate(self.ep_dict["timestamp"]) if abs(ts - i * self.timer_period_2) > 0.002]
            invalid_diffs_2 = [i for i, diff in enumerate(diffs) if abs(diff - self.timer_period_2) > 0.002]
...
@hydra.main(version_base=None, config_path="../config", config_name="record_cfg")
def main(cfg):
