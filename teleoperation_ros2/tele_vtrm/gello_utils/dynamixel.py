...
from tele_vtrm.gello_utils.robot import Robot
...
class DynamixelRobot(Robot):
    ...
    def __init__(
        ...
        from tele_vtrm.gello_utils.dynamixel_driver import (
            DynamixelDriver,
        ...
        if real:
            self._driver = DynamixelDriver(joint_ids, port=port, baudrate=baudrate, gripper_config=gripper_config)
    ...
    def get_joint_state(self) -> np.ndarray:
        pos = (self._driver.get_joints() - self._joint_offsets) * self._joint_signs     #主端关节相对于初始位置的旋转角度
        ...
        return np.append(pos, self._driver.get_gripper())
    ...
    def enable_torque(self, dxl_id,torque):
        self._driver.enable_torque(dxl_id,torque)

    def read_position(self, dxl_id):
        return self._driver.read_position(dxl_id)
    
    def write_position(self, dxl_id, position: Any = None):
        self._driver.write_position(dxl_id, position)
