...
class InferNode(Node):
    ...
    def __init__(self, cfg):
        ...
        self.tac_02_sub
        self.tac_03_sub = self.create_subscription(Image, '/sensor_03/DotView_pp_gray_img', self.tac_03_callback, 1, callback_group=cb_group_1)
        self.tac_03_sub
        self.tac_04_sub = self.create_subscription(Image, '/sensor_04/DotView_pp_gray_img', self.tac_04_callback, 1, callback_group=cb_group_1)
        self.tac_04_sub
        # self.signal = 0
        self.start_ep = False
        # self.end_ep = False
        ...
        self.policy = DiffusionPolicy.from_pretrained(cfg.pretrained_policy_path, map_location=cfg.device)
        torch._dynamo.reset()
        torch.set_float32_matmul_precision("high")
        # self.policy.diffusion.unet = torch.compile(self.policy.diffusion.unet)
    ...
    def tac_03_callback(self, msg):
        img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
        self.obs_dict["observation.images.tac_03"] = self.np_to_torch(img)

    def tac_04_callback(self, msg):
        img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
        self.obs_dict["observation.images.tac_04"] = self.np_to_torch(img)
    ...
    def np_to_torch(self, array):
        ...
        array = array.unsqueeze(0)
        array = array.to(self.cfg.device, non_blocking=True)
        return array
