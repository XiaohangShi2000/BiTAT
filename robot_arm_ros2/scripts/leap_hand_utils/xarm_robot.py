...
class XArmRobot(Robot):
    ...
    def get_joint_state(self) -> np.ndarray:    #六个关节(+夹爪)
        state = self.get_state()
        gripper = state.gripper_pos()
        all_dofs = np.concatenate([state.joints(), gripper])
        return state.joints()

    def command_joint_state(self, joint_state: np.ndarray) -> None:     #更新从端机械臂的关节角度
    ...
    def get_state(self) -> RobotState:  #xyz+六个关节+轴角
    ...
    def get_observations(self) -> Dict[str, np.ndarray]:
        state = self.get_state()
        pos_quat = np.concatenate([state.cartesian_pos(), state.quat()])    #xyz+四元数
        joints = self.get_joint_state()   #六个关节(+夹爪)
        return {
