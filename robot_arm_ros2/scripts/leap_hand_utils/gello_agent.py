...
from leap_hand_utils.agent import Agent
from leap_hand_utils.dynamixel import DynamixelRobot
...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT94VPII-if00-port0": DynamixelRobotConfig(
        ...
        gripper_config=(490, 530),
    ),
    # xArm_left
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT9HDF13-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6),
        joint_offsets=(
            2 * np.pi / 2,
            2 * np.pi / 2,
        ...
        joint_signs=(1, 1, 1, 1, 1, 1),
        gripper_config=(500, 520),
    ),
    # "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT94EV9L-if00-port0": DynamixelRobotConfig(
    #     joint_ids=(1, 2, 3, 4, 5, 6),
    #     joint_offsets=(
    #         2 * np.pi / 2,
    #         1 * np.pi / 2,
...
class GelloAgent(Agent):
    ...
    #     return self._robot.readgripper()
    def enable_torque(self, dxl_id, torque):
        self._robot.enable_torque(dxl_id, torque)
    
    def read_position(self, dxl_id):
        return self._robot.read_position(dxl_id)
    
    def write_position(self, dxl_id, position):
        self._robot.write_position(dxl_id, position)

    def closeport(self):
        self._robot.closeport()